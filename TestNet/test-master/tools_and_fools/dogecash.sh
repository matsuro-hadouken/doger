...
# docker exec -u 0 -it $CONTAINER_NAME $COIN_NAME.sh <MASTERNODE_PRIVATE_KEY> <YOU_EXTERNAL_IP>

# for seeder: docker exec -u 0 -it $CONTAINER_NAME $COIN_NAME.sh <seed> <YOU_EXTERNAL_IP>
...
if [ -z "$1" ] || [ -z "$2" ]; then
    ...
    echo "docker exec -u 0 -it $CONTAINER_NAME $COIN_NAME.sh PRIV_KEY EXTERNAL_IP" && echo
    echo -e "${GREEN}For seeder mode:${NC} docker exec -u 0 -it $CONTAINER_NAME $COIN_NAME.sh ${GREEN}seed${NC}" && echo
...
echo "masternodeaddr=$2:41001" >>$DAEMON_CONFIG && echo >>$DAEMON_CONFIG
